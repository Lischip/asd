...
\textcolor{red}{Lisette - have you interpreted the results of validation anywhere in terms of which ranges the model is valid within, or does this still need to be done?}
...
\begin{itemize}
    ...
    \item Most of the causes of campylobacteriosis originate from the environment, and not from the consumption of meat. This is conform the literature.
    ...
    \item The Cost of Illness is lower than the literature used to validate, this is probably due to the fact that we only look at three chronic cases and the acute cases.
\end{itemize}
...
We conducted extreme conditions tests on multiple values. We only took into consideration their effect on \textit{symptomatic infections}. This yielded the results shown in Table~\ref{tab:extreme_conditions}.

\begin{longtable}[c]{l|l}
\caption{Results of the extreme conditions test}
\label{tab:extreme_conditions} \\
Variable & Range \\
\hline
\endfirsthead
%
...
More details on the policies and on their implementations can be read in Appendix~\ref{ch:det_policies}.