...
Cost of Illness &
%$F(t) = G(t) \times b \times \dfrac{\sum_{k=0}^{48} p(t-k\times\delta)}{t} $
% \[ F(t) =
%   \begin{cases}
%   G(t) \times b \times p(t)  & \quad \text{if } t \text{ is beginning of week}\\
%     G(t) \times b & \quad \text{else}
%   \end{cases}
% \]
...
%Symptomatic infections $F(t)$ with $G(t)$ the stock of \textit{campylobacter} cases, $b$ the rate of symptomatic cases, $p(t)$ is a rectangular pulse with the width of the time step, with $\delta$ being the amount of time between pulses. The Pulse Train only activates at the end of a year, so $k$ is the amount of timesteps in a year.

  %Convert time unit from week to year: Pulse train empties values from the stock once per year
  \\ \hline
\end{tabular}
\label{tab:equations}
\end{table}
...
\iffalse
\textbf{Extra}\\
\textcite{vlaanderen_staat_2019} and curperus also show the actual human infections confirmed by laboratories from 2006-2018, and the numbers for the Netherlands. I assumed these were uninteresting, since we also care about untested individuals
...
The structural and parametric uncertainties resulted in five policies that were be tested. These policies were designed based on reality and are capable of providing a view of the policies' cost effectiveness and robustness. 
...
\begin{itemize}
    ...
    \item Pest control measures: when average temperatures are above 20 \degree Celsius, the government can initiate fly extermination campaigns to limit the spread of diseases.
